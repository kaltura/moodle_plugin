...
"use strict";
  ...
  var Resumable = function(opts){
    ...
    $.uploadNextChunk = function(){
      var found = false;
      // the Kaltura uploadToken.upload action needs the first chunk to be the first to arrive
      // therefore, return false while file.chunks[0].status()=='uploading'
        var firstStillUploading = false;
        $h.each($.files, function(file){
          if(file.chunks.length && file.chunks[0].status()=='uploading' && file.chunks[0].preprocessState !== 2) {
            firstStillUploading = true;
            return(false);
          }
        });
	...
	var successfulChunks = file.successfulChunks();
        if(!file.isComplete() && file.isPaused()===false){
